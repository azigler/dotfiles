...
# (dotfiles-mlti). Events:
#   detected | recovered | check-failed | check-skipped | settings-drift
#
...
# --- state ------------------------------------------------------------------
TRIES=0; PENDING=0; LAST_EMIT=0; LAST_CFAIL=0; LAST_RETRY=0; DERIVED=""; SDRIFT=0
...
write_state() {
  ...
  printf 'tries=%s\npending=%s\nemit=%s\ncfail=%s\nretry=%s\nderived=%s\nsdrift=%s\n' \
    "$TRIES" "$PENDING" "$LAST_EMIT" "$LAST_CFAIL" "$LAST_RETRY" "$DERIVED" "$SDRIFT" > "$tmp" || { rm -f "$tmp"; return 0; }
...
# NOTE (8x8l): the old `[ -r "$TRANSCRIPT" ] || check_failed "no-transcript"`
# line stood here. It now lives inside read_facts() as READ_FAIL, and the
# decision it drove is taken after EXPECTED resolves — so a blind row names the
# seat, and an `off`/undeclared seat emits nothing at all.
