---
description: Orchestrator-side harness for driving an open-ended technical arc autonomously — dispatch parallel research subagents, verify their claims empirically, scrutinize, fold findings into beads + GUARDRAILS + a `refs/research/` archive, then layer the next questions without waiting to be prompted. Distinct from /grok (in-repo code reading), /dive (multi-source compile to Asana), /spec, /check.
