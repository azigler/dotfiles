...
sake) and the three hard guards. Read it in full before prompting the
`Novel opportunities` ideation agent; it is what keeps the ideation off the modal
tie-back. `/desk` and `/elevate` point at the same file — never re-copy it here.
...
4. **Assemble `FINDINGS.md`** = the subagent's take (body + `Novel opportunities`) + the connective `Related explorations` + the frontmatter. Write the `what:` yourself here (orchestrator step — this is where the gitflow tick's bloat slipped through): a **tight 1–2 sentence hook** — what it is + why it's interesting — **never** an abstract, **never** a forced harness tie-back (see `.exploration-meta/README.md` "The `what:` discipline"). A harness/active-arc note in the *body* is conditional the same way: include only when genuine; otherwise "no harness tie-back — interesting on its own terms," never manufactured.
   ...
   **Card corrections get NO durable target, and that is a decision, not an omission (`explore-bhh0`, 2026-08-04).** Corrections to the **Asana Vibes card** do not go back to Asana (Zig's call, 2026-07-31), and they do not get a local record either: they stay in `refs/card-claims-audit.md` and FINDINGS prose. The tempting alternative — write the inline erratum into the card CAPTURE, `refs/vibes-card-<GID>.md`, making `upstream:` just a refs path and costing no new mechanism — was **rejected as actively harmful**. `verify-quotes.py` greps every FINDINGS quoted span against `refs/`, so annotating a capture lets a quote match **our own annotation** instead of the source: exactly the self-certification contamination `explore-msrc` removed by dropping derived and `generator:`-headed files from the haystack, re-manufactured in the one place that new guard cannot see (a card capture carries neither qualifier). The verbatim-mirror objection is therefore a **soundness** argument, not a purity one. Two supports: a card is a **transient input** consumed once by the dive, so it has no future reader to protect; and the card errors that DO have a downstream reader are **digest errors** — cards are minted from the digest, and `agent-authored-harness`'s two card-citation defects traced upstream verbatim to `digests/2026-08-03.md` — which the mechanism above already covers. What is left is card-only noise ("Leptop" for "Lepton" in a one-shot instruction). `refs/card-claims-audit.md` remains the provenance record, and is itself `derived`, so it is already out of the quote haystack — consistent, not accidental.

   **Historical** entries are measured, never backfilled (`explore-gi0p`): if you find an old uncorrected record, file a bead saying so; do not rewrite the digest. **One narrow exception, decided on `explore-jnv8`:** a **wrong fact about a named person's authorship, or a false factual claim, in a published record** may be corrected in place with the same inline-erratum + declaration mechanism — because `explore-8w9s` would make that record semantically retrievable and serve it as an answer. Retroactively settling **process state** (unsettled capture markers, missing declarations, bookkeeping) is what gi0p exists to refuse, and stays refused. If you are tempted to fix anything else in an old file while you are there: stop and file a bead.
