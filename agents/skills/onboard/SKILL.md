...
Since `dotfiles-z10i`, `session-start.sh` already injects this at wake — the
same office line and laurels, plus the seat's charter line and a pointer to
the constitution. **If you saw that header at the top of this session, skip
the command**; it is the same read. Run it when you did not (a resumed or
compacted context whose header scrolled away, or a window renamed mid-session).
