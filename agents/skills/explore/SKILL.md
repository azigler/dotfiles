...
2. **`mkdir -p ~/explore/<topic>/refs`** before writing
3. Write each fetched source's distillation as a `refs/<source-slug>.md` (one per primary source) — captures the substance verbatim or paraphrased under fair use
4. Write `FINDINGS.md` as the synthesis — TL;DR, what the thing is, usability verdict, harness incorporation (where applicable), recommendation
5. Write `CLAUDE.md` as the orientation file pointing future-agents at the structure
6. Run Step 5 (POST to Asana) — the Asana update mirrors the FINDINGS.md content, prefixed with the original source URLs preserved
7. Commit the folder in the umbrella's git: `git add <topic> && git commit -m ":seedling: explore: <topic> — <one-line theme>"`
