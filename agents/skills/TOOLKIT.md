...
### /elevate
**Job:** Max-effort, fresh-eyes re-examination of FINISHED work — the generative twin of /scrutinize (hunts MISSED opportunity, not what's WRONG). Targeted (one project/exploration) or sweep (WEEKLY compendium-wide + the explore loop's review touchpoint, pulse-elevate.timer, Fri).
**Fire when:** A baseline-effort pass under-thought something; "elevate X" / "fresh eyes on max"; or the weekly sweep fires (which also reviews the week's new explorations + dispatches /scrutinize on any that drifted).
...
**Anti-pattern:** Running it at baseline effort, or reusing the original/anchored agent — defeats the whole point. Correctness review is /scrutinize, not this. Closes the loop seam: opportunities become `📌 candidate Vibes card` lines on beads → HUMAN-GATED promotion (interactive: offer + add; autonomous sweep: list in the push, never auto-write the board — proxy has no section-add route anyway). Sweep also writes a dated `Novel opportunities (elevate sweep <date>)` section back into each FINDINGS it examines — the BACKFILL for the ~40 pre-2026-06-29 explorations that predate the required-section rule. LEAD with CURIOSITY, not a tie-back (over-anchoring to real projects misses novelty, Zig 2026-07-13): range with no default ranking over NEW ideas + interlinks to explorations AND open-ended beads (feed the sweep the `br` beads, not only FINDINGS) + active-arc (only when real) + interesting-for-its-own-sake; the BUILD projects (Hermes/MUD-golem/local-models) are tabled but the CONCEPTS (agent-sim/simulation/memory/loops) are NOT. "NO adopt" != "NO build" — weigh the transferable METHOD separately from the artifact (a runtime wall for the tool is rarely a wall for the pattern).
