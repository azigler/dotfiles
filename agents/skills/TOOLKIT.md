...
**Prereqs/side-effects:** Creates refs/ + CLAUDE.md + beads (`br init`); templates in reference/templates/; default stack Go pure-static + SQLite(WAL) + DuckDB analytics lens; pairs with `agents/infra.md`. Optional arc: `reference/pwa-cockpit-push.md` — a TLS PWA cockpit with cross-platform Web Push + badges (iPhone/Safari/Chrome, auto-updating) + the act-now watcher (diff-on-swap, in-memory dedup: prime-silent/clear-on-resolution/retry; full-detail since E2E to own devices), worked example harnessd.
...
### /desk
**Job:** The research lab's ALLOCATOR — weekly whole-corpus pass over ~/explore that emits a <=1,200-word chief-of-research RESOURCING MEMO to Zig (`refs/desk/<date>.md`). The READ half of a compendium that had only a write half. Replaces /elevate's old weekly sweep (Mode B).
**Fire when:** `pulse-desk.timer` (Fri); or "run the desk" / "what should the lab work on next".
**Prereqs/side-effects:** BIG-CONTEXT/FEW-TURNS is the spec — **<=20 assistant turns, corpus in <=4 BULK reads** (file-by-file costs ~7x the loop it summarizes, dotfiles-uxj9). Writes `refs/desk/<date>.md` + `refs/desk/field-notes.md`, `desk:`-prefixed interlinked beads, a `"row":"desk"` pulse-ledger row with a re-run `kind:cmd` proof, dated `Novel opportunities (desk pass <date>)` appends to FINDINGS; push-notifies the top ask + the PATH. Never Asana.
**Anti-pattern:** `br list --json` on the whole backlog (60k tokens of identical tie-backs — it MANUFACTURES the over-anchoring this loop counters; titles in, `br show` on demand). A memo with no §2 WHAT TO STOP (a chief who only proposes isn't allocating). A ranked menu instead of a POSITION — Zig walks INTO this artifact to drive next steps, so it must take a stand and live on disk, not in the tick's transcript (explore-b47q). Auto-closing the "beads silently answered" (propose only — allocation is the owner's job). Uncited claims (a bad pitch spends trust). "Nothing to report" on an empty week (corpus-level signal still exists). Migrating the archived `elevate` ledger rows (they stay under the old name — explore-b47q).
...
**Anti-pattern:** HTML in Asana fields; abstract image subjects (force specificity via the randomize roll). A dive that never surfaces — the verb is BOUNDED; that IS the time-box. Archival mode: skipping Step 3.5 (consult the compendium — INDEX.md + grep prior FINDINGS) or omitting the now-REQUIRED `Related explorations` + `Novel opportunities` sections — the two that make the double loop compound (both were skipped historically; audit explore-doj). End the Asana report with the `Archived: ~/explore/<topic>/` back-pointer. LEAD the Novel-opportunities pass with CURIOSITY, not a tie-back — over-anchoring to the real projects here misses novelty (Zig 2026-07-13). Range with NO default ranking over: entirely-NEW ideas (tied to nothing); interlinks to other explorations AND open-ended beads (deeper contexts); active-arc applications (only when real, by recency — NOT privileged); interesting-for-its-own-sake. The BUILD PROJECTS (Hermes/MUD-golem/local-coding-models) are tabled, but the CONCEPTS they touched (agent-sim/simulation/memory/loops) are NOT — never write "tabled agent-sim arc" or call a sim/pet build "not a live destination." Don't manufacture a tie-back. Mirrors /desk's and /elevate's "Where the upside can land"; keep all three in sync. ALSO: "NO adopt" is NOT "NO build" — hold apart "adopt this artifact?" vs "is there a transferable METHOD/pattern/primitive?" (a runtime wall for the tool is rarely a wall for the pattern); a verdict that jumps "runtime mismatch"->"no build" is the bug (buried colibri/kapa/ternlight builds, 2026-07-13).
